...
def rds_instance_scheduler(name:str):
  try:
    ...
    if db_status == 'available':
      logger.info("Stopping RDS instance: %s", rds_instance['DBInstances'][0].get('DBInstanceIdentifier'))
      rds.stop_db_instance(DBInstanceIdentifier=name)
    elif db_status == 'stopped':
      logger.info('Starting RDS instance: %s', rds_instance['DBInstances'][0].get('DBInstanceIdentifier'))
...
def rds_cluster_scheduler(name:str):
  try:
    ...
    if db_status == 'available':
      logger.info("Stopping RDS cluster: %s", rds_cluster['DBClusters'][0].get('DBClusterIdentifier'))
      rds.stop_db_cluster(DBClusterIdentifier=name)
    elif db_status == 'stopped':
      logger.info('Starting RDS cluster: %s', rds_cluster['DBClusters'][0].get('DBClusterIdentifier'))
...
def ecs_cluster_scheduler(name:str, count:int):
  try:
    ...
    for service in service_list:
      service_details = ecs.describe_services(cluster=name, services=[service])['services'][0]
