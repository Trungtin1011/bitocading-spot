...
def list_available_services(profile:str):
  try:
    return [svc for svc in boto3.Session(profile_name=profile).get_available_services()]
...
def list_bucket_objects(bucket:str):
  try:
    return [object['Key'] for object in boto3.client("s3").list_objects(Bucket=bucket)['Contents']]
  except Exception as e:
    logger.error(e.args[0])
    return e.args[0]
...
def list_bucket_tags(bucket:str):
  try:
    return [tags['Key'] + ": " + tags['Value'] for tags in boto3.client("s3").get_bucket_tagging(Bucket=bucket)['TagSet']]
  except Exception as e:
    logger.error(e.args[0])
    return e.args[0]
...
def list_bucket_through_vpce(region:str,endpoint:str):
  try:
    s3_client = boto3.client(service_name="s3", region_name=region, endpoint_url=endpoint)
...
def check_ec2_tag_exist(instance_id:str,tag:str):
  try:
    ec2_instance = boto3.client('ec2').describe_instances(InstanceIds=[instance_id])['Reservations']
