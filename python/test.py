...
def solution(A):
	res = -1
	if all(isinstance(ele, list) for ele in A) and len(A) in range(1, 1001) and len(A[0]) in range(1, 1001) \
		and min(map(min, A)) >= 1 and max(map(max, A)) <= len(A)*len(A[0]):
		res = 0
		arr = []
		for item in A:
...
def splitWay(S):
	result = -1
	if len(S) in range(2, 2*int(1e+5)+1) and S.islower():
		result = 0
		for i in range(1,len(S)):
			if S[0:i].count("x") == S[0:i].count("y") or S[i:].count("x") == S[i:].count("y"):
				#print(i)
				result+=1
	return result
...
#########################################################################################################
def delWay(S):
	if ((S.count('A') + S.count('B')) == len(S)) and 1 <= len(S) and len(S) <= int(1e+5)+1:
		nA = S.count('A')
		nB = S.count('B')
		if nA == len(S) or nB == len(S):
			...
			count += min([S[S.find('B'):].count('A'), S[S.find('B'):S.rfind('A')].count('B')])
...
def solution(A,K):
	if len(A) in range(1,201) and K in range(0, int(1e+9)+1) and min(A) >= 0 and max(A) <= int(1e+9):
		n = len(A) // K
		B = []
		for i in range(n):
...
def solution(A, B):
	result = -1
	if A in range(1, int(1e+9)+1) and B in range(1, int(1e+9)+1):
		result = 0
		result_arr = [A//4, B//4]
		if A//3 <= B:
...
def solution(D, C, P):
	result = -1
	if len(D) == len(C) and len(C) in range(1, int(1e+5)+1) and P in range(0, int(1e+9)+1):
		for n in range(len(D)):
			if D[n] not in range(1,int(1e+9)+1) or C[n] not in range(1, int(1e+9)+1): return -1
		
		result = 0
		if min(C) > P: return result
		if len(set(D)) == 1:
			C.sort()
		else:
			DC_map = dict(zip(D, C))
